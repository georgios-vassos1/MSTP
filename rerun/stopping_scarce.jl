...
q(v,p)=sort(v)[clamp(ceil(Int,p*length(v)),1,length(v))]
# SCARCE + correlated regime: capacity binds, recourse matters -> SDDP must learn.
cfg=loadcfg("capreg_l20u95"; lambda=20.0, rho=0.4)
println("instance capreg_l20u95: tau=$(cfg.tau) nO=$(cfg.nOrigins) nD=$(cfg.nDestinations) meancap=$(round(mean(cfg.carrier_capacity),digits=2))"); flush(stdout)
for it in (10,25,50,100,250,500,1000)
  m=train_model(cfg,Int64(it)); s=simulate_model(m,cfg,Int64(300)); obj=s["obj"]; noise=s["noise"]; r=Float64[]
  ...
  println(">>> iters=$it regret mean=$(round(mean(r),digits=2))% med=$(round(median(r),digits=2))% q25=$(round(q(r,0.25),digits=2))% q75=$(round(q(r,0.75),digits=2))%"); flush(stdout)
end
println("DONE")
